...
def flag(subcomplex,maxdim=None):
    ...
    #Basically finds all the graph vertices.
    verts=list(set([v for s in subcomplex for v in s]))
    ...
    #Each pass adds all cliques (not just maximal cliques) of increasing dimension in the graph
    while added and ((maxdim == None) or (k<=maxdim)):
        added=kflag(subcomplex,k,verts=verts)
        #printAS(added,("Added to the complex for k = " + str(k)))
        complex+=added
        k+=1
    return complex
...
def boundary(toplexes,k,relative=None):
    ...
    for i,km1spx in enumerate(km1chain):
        for j,kspx in enumerate(kchain):
           parity=1
           for boundary in ksublists(kspx,k): ## ksublist = k-1 chain
               parity*=-1
...
""" Praggastis added: """
def excludedSet(toplexes,k,simplices):
    """Returns a the list of simplices which will be identified with * for homology group calculation"""
    rel=[spx for spx in (ksimplices(toplexes,k)+ksimplices(toplexes,k-1)) 
         if not any([set(simplex).issubset(spx) for simplex in simplices])]
    return rel
